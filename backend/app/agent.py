...
directly to an end, or through user-defined condition/branch nodes that
gate entry to the agent (see `_build_graph`). All nodes emit events via
...
from .schemas import FALSE_BRANCH, TRUE_BRANCH
...
def _build_graph(llm_client: LLMClient, tools: list[dict], graph: dict) -> CompiledStateGraph:
    ...
    exactly one, enforced by WorkflowGraph validation). Condition nodes are
    compiled into real nodes with conditional edges, resolved recursively so
    a chain of conditions or a direct wire straight to an end all work the
    same way. `resolve` is called lazily per node id and memoizes results so
    a node shared by multiple incoming edges is only added once.
    """
